...
{   
    ...
    {   
        ...
        Ipv4InterfaceContainer ip_s_r1[i] = address.Assign(s_r1[i]);

        std::string ip2 = "10.1."+std::to_string(i+2+nSources)+".0";
        address.SetBase(ip2.c_str(), "255.255.255.0");
        Ipv4InterfaceContainer ip_d_r2[i] = address.Assign(d_r2[i]);
    }
    ...
    for (uint32_t i = 0; i < nSources; i++)
    {
        ...
        app[i]->Setup (ns3TcpSocket[i], sinkAddress[i], pktSize, nPackets, DataRate ("1Mbps"));
    ...
    // only tracing the cwnd for a single source i.e. source -> 1 ( node - 2).
    ns3TcpSocket[2]->TraceConnectWithoutContext ("CongestionWindow", MakeBoundCallback (&plotCwndChange, stream));
    
    
    for (uint32_t i = 0; i < nSources; i++)
    {
        d_r2[i].Get (0)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback (&RxDrop));
    }
